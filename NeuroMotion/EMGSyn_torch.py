...
from NeuroMotion.MNPoollib.MNPoolStatus_torch import MotoneuronPoolStatus
...
class EMGSynthesiser:
    def __init__(
        ...
        self.emg = torch.zeros((10, 32, win_len + 96), **self.dev_dtype)
        self.full_emg = []

    def _init_mn_pool(self, MNPool, mnpool_kwargs):
        self.mn_pool = {}
        for k, v in mnpool_kwargs.items():
            mn_pool = MNPool(**{**v, **self.dev_dtype})
            num_mu = mn_pool.N

            properties = mn_pool.assign_properties()
            mn_pool.num = properties["num"].reshape(num_mu, 1)
            mn_pool.depth = properties["depth"].reshape(num_mu, 1)
            mn_pool.angle = properties["angle"].reshape(num_mu, 1)
            mn_pool.iz = properties["iz"].reshape(num_mu, 1)
            mn_pool.cv = properties["cv"].reshape(num_mu, 1)
            mn_pool.length = properties["len"].reshape(num_mu, 1)
    ...
    def _init_muap_latent(self, mnpool_kwargs, biomime_cfg):
        self.muap_latent = {}
        for k, v in mnpool_kwargs.items():
            self.muap_latent[k] = torch.randn(
                v["N"], biomime_cfg.Model.Generator.Latent, **self.dev_dtype
            )
    ...
    def generate_muap(self, muscle_lengths):
        ...
        for ms, pool in self.mn_pool.items():
            ...
            with torch.no_grad():
                sim = self.generator.sample(num_mu, cond, cond.device, zi)
            muaps[ms] = sim.permute(0, 2, 3, 1)
    ...
    def generate_spikes(self, activations, dt=None):
        ...
        for ms, pool in self.mn_pool.items():
            current_spikes = pool.generate_current_spikes(activations[ms], self._fs, dt)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cfg", type=str, default="config.yaml", help="Name of configuration file"
    )
    parser.add_argument(
        "--model_pth",
        default="./ckp/model_linear.pth",
        type=str,
        help="path of best pretrained BioMime model",
    )
